...
# Arena geometries this branch can be built for (constants.h
# panel_count_per_frame_row / _col, selected by -DARENA_HW_10_10 / _12_18):
# G6_4x10 (arena_10-10) or G6_4x12 (arena_12-18).
EXPECTED_GEOMETRIES = {(4, 10), (4, 12)}
...
def test_firmware_version_reply_shape(transport):
    ...
    assert (v.rows, v.cols) in EXPECTED_GEOMETRIES, f"unexpected arena {v.rows}x{v.cols}"
    assert 0 <= v.flags <= (FLAG_DIRTY | FLAG_DEBUG), f"undefined flag bits set: {v.flags:#04x}"
